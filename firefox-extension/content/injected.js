...
(function() {
    ...
    const DEFAULT_STORE_URL = 'https://addons.mozilla.org/en-US/firefox/addon/strava-feed-filter/';
    ...
    const UpdateModule = {
        async init(panel) {
            ...
            let firefoxInfo = remoteData && remoteData.firefox;
            if (!firefoxInfo && FORCE_UPDATE_BANNER && currentVersion) {
                firefoxInfo = {
                    latest: currentVersion,
                    store_url: ''
                };
            }
            if (!firefoxInfo || !firefoxInfo.latest) {
                this.hideBanner(panel);
                return;
            }

            const latestVersion = firefoxInfo.latest.trim();
            const storeUrl = firefoxInfo.store_url || '';
